...
THEANO_WEIGHTS_PATH_NO_TOP = '/opt/ml/vgg16_weights_th_dim_ordering_th_kernels_notop.h5'
TF_WEIGHTS_PATH_NO_TOP = '/opt/ml/vgg16_weights_tf_dim_ordering_tf_kernels_notop.h5'

TH_19_WEIGHTS_PATH_NO_TOP = '/opt/ml/vgg19_weights_th_dim_ordering_th_kernels_notop.h5'
TF_19_WEIGHTS_PATH_NO_TOP = '/opt/ml/vgg19_weights_tf_dim_ordering_tf_kernels_notop.h5'
...
parser.add_argument("--image_size", dest="img_size", default='400', type=str,
                    help='Minimum image size')

parser.add_argument("--content_weight", dest="content_weight", default='0.025', type=str,
                    help="Weight of content")

parser.add_argument("--style_weight", dest="style_weight", nargs='+', default=['1'], type=str,
                    help="Weight of style, can be multiple for multiple styles")

parser.add_argument("--style_scale", dest="style_scale", default='1.0', type=str,
                    help="Scale the weighing of the style")

parser.add_argument("--total_variation_weight", dest="tv_weight", default='8.5e-5', type=str,
                    help="Total Variation weight")

parser.add_argument("--num_iter", dest="num_iter", default='1', type=str,
                    help="Number of iterations")

parser.add_argument("--model", default="vgg16", type=str,
                    help="Choices are 'vgg16' and 'vgg19'")

parser.add_argument("--content_loss_type", default='0', type=str,
...
parser.add_argument('--min_improvement', default='0.0', type=str,
...
if len(style_image_paths) != len(args.style_weight):
    print("Mismatch in number of style images provided and number of style weights provided. \n"
          ...
          "Equally distributing weights to all other styles." % (len(style_image_paths), len(float(args.style_weight))))

    weight_sum = sum(float(args.style_weight)) * float(args.style_scale)
    ...
    for style_weight in args.style_weight:
        style_weights.append(float(style_weight) * float(args.style_scale))
...
assert int(args.content_loss_type) in [0, 1, 2], "Content Loss Type must be one of 0, 1 or 2"
...
def preprocess_image(image_path, load_dims=False, read_mode="color"):
    ...
    if load_dims:
        ...
        img_width = int(args.img_size)
        if maintain_aspect_ratio:
            img_height = int(img_width * aspect_ratio)
        else:
            img_height = int(args.img_size)
...
if K.image_dim_ordering() == "th":
    if args.model == "vgg19":
        weights = TH_19_WEIGHTS_PATH_NO_TOP
    else:
        weights = THEANO_WEIGHTS_PATH_NO_TOP
else:
    if args.model == "vgg19":
        weights = TF_19_WEIGHTS_PATH_NO_TOP
    else:
        weights = TF_WEIGHTS_PATH_NO_TOP
...
def content_loss(base, combination):
    ...
    if int(args.content_loss_type) == 1:
        multiplier = 1. / (2. * (channels ** 0.5) * (size ** 0.5))
    elif int(args.content_loss_type) == 2:
